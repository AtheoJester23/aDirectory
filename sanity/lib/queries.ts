...
export const STARTUPS_QUERY = defineQuery(`*[_type == "startup" && defined(slug.current) && !defined($search) || title match $search || category match $search || autor->name match $search] | order(_createdAt desc){
    _id, 
    title, 
    slug, 
    _createdAt, 
...
export const AUTHORS_QUERY = defineQuery(`*[_type == "author" && id == $id]{
